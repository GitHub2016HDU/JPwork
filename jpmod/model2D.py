...
class model2D(object):
    ...
    def conv_op(self, input_op, name, ksize, n_out, stride, p):
        ...
        with tf.name_scope(name) as scope:
            kernel = tf.get_variable(name       =scope+'w',
                                     shape      =[ksize[0], ksize[1], n_in, n_out],
                                     dtype      =tf.float32,
    ...
    def mpool_op(self, input_op, name, ksize, stride):
        return tf.nn.max_pool(  value           =input_op,
    ...
    def inference_op(self, input_op, keep_prob):
        p = []
        conv1_1 = self.conv_op(input_op=input_op, name='conv1_1', ksize=[3, 3], n_out=16, stride=[1, 1], p=p)
        conv1_2 = self.conv_op(input_op=conv1_1, name='conv1_2', ksize=[3, 3], n_out=24, stride=[1, 1], p=p)
        pool1 = self.mpool_op(input_op=conv1_2, name='pool1', ksize=[2, 2], stride=[2, 2])

        conv2_1 = self.conv_op(input_op=pool1, name='conv2_1', ksize=[3, 3], n_out=32, stride=[1, 1], p=p)
        conv2_2 = self.conv_op(input_op=conv2_1, name='conv2_2', ksize=[3, 3], n_out=32, stride=[1, 1], p=p)
        pool2 = self.mpool_op(input_op=conv2_2, name='pool2', ksize=[2, 2], stride=[2, 2])

        conv3_1 = self.conv_op(input_op=pool2, name='conv3_1', ksize=[3, 3], n_out=64, stride=[1, 1], p=p)
        conv3_2 = self.conv_op(input_op=conv3_1, name='conv3_2', ksize=[3, 3], n_out=64, stride=[1, 1], p=p)
        pool3 = self.mpool_op(input_op=conv3_2, name='pool3', ksize=[2, 2], stride=[2, 2])

        conv4_1 = self.conv_op(input_op=pool3, name='conv4_1', ksize=[3, 3], n_out=128, stride=[1, 1], p=p)
        conv4_2 = self.conv_op(input_op=conv4_1, name='conv4_2', ksize=[3, 3], n_out=128, stride=[1, 1], p=p)
        pool4 = self.mpool_op(input_op=conv4_2, name='pool4', ksize=[2, 2], stride=[2, 2])

        shp = pool4.get_shape()
        flattened_shape = shp[1].value*shp[2].value*shp[3].value
        resh1 = tf.reshape(tensor=pool4, shape=[-1, flattened_shape], name='resh1')

        fc5 = self.fc_op(resh1, name='fc5', n_out=32, p=p)
